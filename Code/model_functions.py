...
def getNumAgentPairsWith4Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 4 for a in pwa ])

def getNumAgentPairsWith5Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 5 for a in pwa ])

def getNumAgentPairsWith6Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 6 for a in pwa ])

def getNumAgentPairsWith7Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 7 for a in pwa ])

def getNumAgentPairsWith8Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 8 for a in pwa ])

def getNumAgentPairsWith9Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 9 for a in pwa ])

def getNumAgentPairsWith10Agreements(model):
    pwa = getNumPairwiseAgreements(model)
    return sum([ a == 10 for a in pwa ])
# END UGLY
...
def verifyBucketCount(model):
    agentCount = 0
    for opinions,agents in model.buckets.items():
        agentCount+=len(agents)

    if(agentCount>model.num_agents):
        print("Agent count is greater than the number of agents")
        exit()
...
def plotPolarizationMeasures(model):
        ...
        ax.set_title("CI2: Measures of Issue Alignment")
        ax.set_xlabel('Time (steps)')
